...
class MNISTClassifier_5layer_1(nn.Module):
    def __init__(self, p=0.5):
        super(MNISTClassifier_5layer_1, self).__init__()
        self.fc1 = nn.Linear(784, 512)           # 增加第一层神经元数量
        self.bn1 = nn.BatchNorm1d(512)           # 批归一化
        self.fc2 = nn.Linear(512, 256)           # 增加第二层
        self.bn2 = nn.BatchNorm1d(256)           # 批归一化
        self.fc3 = nn.Linear(256, 128)           # 增加第三层
        self.bn3 = nn.BatchNorm1d(128)           # 批归一化
        self.fc4 = nn.Linear(128, 64)            # 增加第四层
        self.bn4 = nn.BatchNorm1d(64)            # 批归一化
        self.fc5 = nn.Linear(64, 10)             # 输出层
        self.dropout = nn.Dropout(p=p)           # Dropout 层
    ...
    def forward(self, x):
        ...
        x = F.relu(self.bn3(self.fc3(x)))        # 第三层
        x = self.dropout(x)                      # Dropout
        x = F.relu(self.bn4(self.fc4(x)))        # 第四层
        x = self.dropout(x)                      # Dropout
        x = self.fc5(x)                          # 输出层
        return x
