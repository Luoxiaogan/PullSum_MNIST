...
def compute_accuracy(model_class, model_list, testloader):
...
def train_PullSum(
    ...
    h_data_train = [tensor.to(device) for tensor in h_data_train]
    y_data_train = [tensor.to(device) for tensor in y_data_train]
    ...
    for epoch in progress_bar:
        ...
        for batch_idx, batch in enumerate(
            zip(*[iter(loader) for loader in trainloader_list])
        ):
            inputs = [data[0] for data in batch]
            labels = [data[1] for data in batch]
            h_data_train = [tensor.to(device) for tensor in inputs]
            y_data_train = [tensor.to(device) for tensor in labels]
