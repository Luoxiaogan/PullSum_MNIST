...
def train_PullDiag(
    ...
    if show_graph:
        ...
        plt.suptitle(f"PullDiag, n={n}, lr={lr:.6f}, batch_size={batch_size}")
        plt.tight_layout(rect=[0, 0.03, 1, 0.90])  # 调整顶部边距
        plt.show()
...
def train_FRSD(
    ...
    optimizer = FRSD(model_list=model_list, lr=lr, beta=beta, A=A, closure=closure)
    ...
    if show_graph:
        ...
        plt.suptitle(f"FRSD, n={n}, lr={lr:.6f}, batch_size={batch_size}")
        plt.tight_layout(rect=[0, 0.03, 1, 0.90])  # 调整顶部边距
        plt.show()
...
def train_FROZEN(
    ...
    optimizer = FROZEN(model_list=model_list, lr=lr, beta=beta, A=A, closure=closure)
    ...
    if show_graph:
        ...
        plt.suptitle(f"FROZEN, n={n}, lr={lr:.6f}, batch_size={batch_size}")
        plt.tight_layout(rect=[0, 0.03, 1, 0.90])  # 调整顶部边距
        plt.show()
